...
export default function LinkModal({ onClose, onCreated }: LinkModalProps) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto p-6 space-y-6">
          {activeTab === 'basic' ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex">
                  <span className="inline-flex items-center px-4 rounded-l-xl border border-r-0 border-border bg-secondary text-muted-foreground text-sm">
                    {process.env.NEXT_PUBLIC_BASE_URL?.replace('http://', '').replace('https://', '') || 'link.co'}/
                  </span>
                  <input 
                    {...register('customSlug')}
                    className="flex-1 bg-input border border-border rounded-r-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary"
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 rounded-xl border border-border bg-secondary/20">
                ...
                {geoFields.map((field, index) => (
                  <div key={field.id} className="flex gap-2 mb-2">
                    <input 
                      {...register(`geoTargeting.${index}.country`)}
                      placeholder="Country Code (e.g., US)"
                      className="w-24 bg-input border border-border rounded-lg px-3 py-2 text-sm"
                    />
                    <input 
                      {...register(`geoTargeting.${index}.url`)}
              ...
              <div className="p-4 rounded-xl border border-border bg-secondary/20">
                ...
                <div className="space-y-2">
                  <input 
                    {...register('deviceTargeting.mobile')}
                    placeholder="Mobile URL Redirect"
                    className="w-full bg-input border border-border rounded-lg px-3 py-2 text-sm"
                  />
                  <input 
                    {...register('deviceTargeting.desktop')}
                    placeholder="Desktop URL Redirect"
                    className="w-full bg-input border border-border rounded-lg px-3 py-2 text-sm"
                  />
                </div>
              </div>
          ...
          <div className="pt-4 mt-6 border-t border-border flex justify-end gap-3">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="bg-primary hover:bg-primary/90 text-primary-foreground px-5 py-2.5 rounded-xl font-medium flex items-center gap-2 shadow-lg shadow-primary/20 transition-all disabled:opacity-70"
            >
